...
def main():
    ...
    # IndoWiki
    wiki = load_dataset("sabilmakbar/indo_wiki", "indowiki_dedup_all", split="su", trust_remote_code=True)
    wiki = wiki.remove_columns([col for col in wiki.column_names if col != "text"])
    all_datasets.append(wiki)

    # KoPI-NLLB
    kopi = load_dataset("acul3/KoPI-NLLB", "sun_Latn-neardup", split="train", trust_remote_code=True)
    kopi = kopi.remove_columns([col for col in kopi.column_names if col != "text"])
    all_datasets.append(kopi)

    # CulturaX
    culturax = load_dataset("uonlp/CulturaX", "su", split="train", trust_remote_code=True)
    culturax = culturax.remove_columns([col for col in culturax.column_names if col != "text"])
    all_datasets.append(culturax)

    raw_datasets = concatenate_datasets(all_datasets)
